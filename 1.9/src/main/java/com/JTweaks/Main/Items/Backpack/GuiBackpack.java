...
/*	
	public GuiBackpack(IInventory playerInv, InventoryBackpack te) {
	    super(new ContainerBackpack(playerInv, te));
	    this.xSize = 176;
	    this.ySize = 166;
	    }
	...
	private static final ResourceLocation iconLocation = new ResourceLocation(Reference.MODID + ":" + "textures/gui/Backpack.png");
	...
	protected void drawGuiContainerBackgroundLayer(float par1, int par2, int par3)
	{
		...
		this.mc.getTextureManager().getTexture(iconLocation);
	...
	public static void drawPlayerModel(int par0, int par1, int par2, float par3, float par4, EntityLivingBase par5EntityLivingBase)
	{
		...
		GL11.glTranslatef(0.0F, par5EntityLivingBase.renderYawOffset, 0.0F);
		
		try {
			RenderManager.class.newInstance().playerViewY = 180.0F;
		...
		//---------------------------------------------------//
		try {
			RenderManager.class.newInstance().doRenderEntity(par5EntityLivingBase, 0.0D, 0.0D, 0.0D, 0.0F, 1.0F, true);
		} catch (InstantiationException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		} catch (IllegalAccessException e) {
